/* linux/arch/arm/mach-msm/board-primoc-wifi.c
 *
...
int primoc_wifi_set_carddetect(int on);
int primoc_wifi_get_mac_addr(unsigned char *buf);
...
static void *primoc_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init primoc_init_wifi_mem(void)
{
	int i;
...
static struct resource primoc_wifi_resources[] = {
	[0] = {
		.name		= "bcm4329_wlan_irq",
		.start		= MSM_GPIO_TO_INT(PRIMOC_GPIO_WIFI_IRQ),
		.end		= MSM_GPIO_TO_INT(PRIMOC_GPIO_WIFI_IRQ),
#ifdef HW_OOB
...
static struct wifi_platform_data primoc_wifi_control = {
	.set_power      = primoc_wifi_power,
	.set_reset      = primoc_wifi_reset,
	.set_carddetect = primoc_wifi_set_carddetect,
	.mem_prealloc   = primoc_wifi_mem_prealloc,
	.get_mac_addr	= primoc_wifi_get_mac_addr,
	.dot11n_enable  = 1,
};

static struct platform_device primoc_wifi_device = {
				.name           = "bcm4329_wlan",
				.id             = 1,
				.num_resources  = ARRAY_SIZE(primoc_wifi_resources),
				.resource       = primoc_wifi_resources,
				.dev            = {
				.platform_data = &primoc_wifi_control,
				},
};

unsigned char *get_wifi_nvs_ram(void);

static unsigned primoc_wifi_update_nvs(char *str)
{
...
int primoc_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk(KERN_INFO"primoc_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
...
#else
	primoc_wifi_update_nvs("sd_oobonly=1\n");
#endif
	primoc_wifi_update_nvs("btc_params80=0\n");
	primoc_wifi_update_nvs("btc_params6=30\n");
	primoc_init_wifi_mem();
	ret = platform_device_register(&primoc_wifi_device);
	return ret;
}
