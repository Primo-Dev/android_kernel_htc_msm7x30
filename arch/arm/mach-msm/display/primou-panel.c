...
static uint32_t display_on_gpio_table[] = {
	...
	LCM_GPIO_CFG(primou_LCD_R0, 1),
	LCM_GPIO_CFG(primou_LCD_R1, 1),
	LCM_GPIO_CFG(primou_LCD_R2, 1),
	...
	LCM_GPIO_CFG(primou_LCD_G0, 1),
	LCM_GPIO_CFG(primou_LCD_G1, 1),
	...
	LCM_GPIO_CFG(primou_LCD_B0, 1),
	LCM_GPIO_CFG(primou_LCD_B1, 1),
	LCM_GPIO_CFG(primou_LCD_B2, 1),
	...
	//LCM_GPIO_CFG(primou_LCD_RSTz, 1),
};
...
static uint32_t display_off_gpio_table[] = {
	...
	LCM_GPIO_CFG(primou_LCD_R0, 0),
	LCM_GPIO_CFG(primou_LCD_R1, 0),
	LCM_GPIO_CFG(primou_LCD_R2, 0),
	...
	LCM_GPIO_CFG(primou_LCD_G0, 0),
	LCM_GPIO_CFG(primou_LCD_G1, 0),
	...
	LCM_GPIO_CFG(primou_LCD_B0, 0),
	LCM_GPIO_CFG(primou_LCD_B1, 0),
	LCM_GPIO_CFG(primou_LCD_B2, 0),
...
static void primou_sony_panel_power(bool on_off)
{
  if (!!on_off) {
    LCMDBG("(%d):\n", on_off);
...
static void primou_lg_panel_power(bool on_off)
{
  if (!!on_off) {
    LCMDBG("(%d):\n", on_off);
    gpio_set_value(primou_LCD_RSTz, 1);
...
static int panel_power(int on)
{
  ...
  case PANEL_ID_primou_LG:
    ...
    return 0;
    break;
  case PANEL_ID_primou_SONY:
    primou_sony_panel_power(on == 1 ? true : false);
    return 0;
    break;
  }
  return -EINVAL;
...
{
  if (!strcmp(name, "lcdc_primou_wvga"))
    return 0;
}
...
static struct platform_device lcdc_primouwvga_panel_device = {
	.name   = "lcdc_primou_wvga",
	.id     = 0,
	.dev    = {
...
int __init primou_init_panel(void)
{
  ...
  msm_fb_add_devices(
                     primou_fb_devices, ARRAY_SIZE(primou_fb_devices));

  ret = platform_device_register(&lcdc_primouwvga_panel_device);
  if (ret != 0)
    return ret;
  return 0;
>>>>>>> upstream/4.3_primoc
